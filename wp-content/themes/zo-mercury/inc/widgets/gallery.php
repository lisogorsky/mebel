<?php
class Zo_Mercury_Gallery extends WP_Widget {
	...
	function __construct() {
		$this->plugin_slug  = 'zo-gallery';
        parent::__construct(
            $this->plugin_slug, // Base ID
            __('Gallery', 'zo-mercury'), // Name
            array('description' => __('List of your site&#8217;s galleries Portfolio.', 'zo-mercury'),) // Args
        );
		...
		<?php while ( $r->have_posts() ) : $r->the_post(); ?>
			<div class="recent-item col-xs-4 col-sm-4 col-md-4">
				<div class="recent-thumb">
					...
					if ( $show_thumb ) : 
						...
						if ( $only_1st_img ) :
							...
							$img_full = wp_get_attachment_image( $this->get_first_content_image_id( get_the_content() ), 'full' );
						else :
							// else 
							...
							if ( has_post_thumbnail() ) : 
								...
								$image_url = zo_image_resize(wp_get_attachment_image_src(get_post_thumbnail_id(get_the_ID()))[0], $thumb_width, $thumb_height);
					...
					?>
					<a class="prettyPhoto recent-thumb-overlay" rel="prettyPhoto[pp_gal]" href="<?php echo esc_url($img_full); ?>" ><i class="fa fa-plus"></i></a>
				</div>
				<div class="recent-detail">
					...
					<?php if ( $show_date ) : ?> 
						<div class="zo-post-date"><span class="post-author"><i class="fa fa-user"></i> <?php echo get_the_author(); ?></span><span class="post-date"><i class="fa fa-clock-o"></i><?php echo get_the_date('j F, Y');?></span></div>
					<?php	endif; ?>
				</div>
			</div>
		<?php endwhile; ?>
...
<?php
	...
	function form( $instance ) {
		...
		$hide_title   = isset( $instance[ 'hide_title' ] ) ? (bool) $instance[ 'hide_title' ] : true;
		...
		?>
		<p><label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php _e( 'Title:', 'zo-mercury' ); ?></label>
		<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr($title); ?>" /></p>

		<p><label for="<?php echo esc_attr($this->get_field_id( 'number_posts' )); ?>"><?php _e( 'Number of posts to show:', 'zo-mercury' ); ?></label>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'show_date' )); ?>"><?php _e( 'Display post date?', 'zo-mercury' ); ?></label></p>

		<h4><?php _e( 'Thumbnail Options', 'zo-mercury' ); ?>:</h4>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'show_thumb' )); ?>"><?php _e( 'Display post featured image?', 'zo-mercury' ); ?></label></p>
		
		<p><label for="<?php echo esc_attr($this->get_field_id( 'thumb_width' )); ?>"><?php _e( 'Width of thumbnail', 'zo-mercury' ); ?>:</label>
		...
		<p><label for="<?php echo esc_attr($this->get_field_id( 'thumb_height' )); ?>"><?php _e( 'Height of thumbnail', 'zo-mercury' ); ?>:</label>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'keep_aspect_ratio' )); ?>"><?php _e( 'Use aspect ratios of original images?', 'zo-mercury' ); ?> <em><?php _e( 'If checked the given width is used to determine the height of the thumbnail automatically. This option also supports responsive web design.', 'zo-mercury' ); ?></em></label></p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'hide_title' )); ?>"><?php _e( 'Do not show title?', 'zo-mercury' ); ?> <em><?php _e( 'Make sure you set a default thumbnail for posts without a thumbnail, otherwise there will be no link.', 'zo-mercury' ); ?></em></label> </p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'try_1st_img' )); ?>"><?php _e( "Try to use the post's first image if post has no featured image?", 'zo-mercury' ); ?></label></p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'only_1st_img' )); ?>"><?php _e( 'Use first image only, ignore featured image?', 'zo-mercury' ); ?></label></p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'use_default' )); ?>"><?php _e( 'Use default thumbnail if no image could be determined?', 'zo-mercury' ); ?></label></p>
		
		<p><label for="<?php echo esc_attr($this->get_field_id( 'default_url' )); ?>"><?php _e( 'URL of default thumbnail (start with http://)', 'zo-mercury' ); ?>:</label>
 ...
 * @since 1.0
 */
function zo_mercury_register_gallery () {
	register_widget('Zo_Mercury_Gallery');
}
add_action('widgets_init', 'zo_mercury_register_gallery');