...
class WC_Widget_Cart_Search extends WP_Widget {
    public function __construct() {
        parent::__construct(
            ...
            __('Cart & Search', 'zo-mercury'), // Name
            array('description' => __("Display the user's Cart and Search form in the sidebar.", 'zo-mercury'),) // Args
        );
    }
                        ...
                        <div class="shopping_cart_dropdown_inner">
                            ...
                            <ul class="cart_list product_list_widget">
                                ...
                                <?php else : ?>
                                    ...
                                    <li class="cart-list clearfix empty"><?php _e( 'No products in the cart.', 'zo-mercury' ); ?></li>
                            ...
                            <?php if ( sizeof( $woocommerce->cart->get_cart() ) > 0 ) : ?>
                                <div class="cart-total">
                                    <span class="total"><?php _e( 'Sub Total', 'zo-mercury' ); ?>:<span><?php echo do_shortcode($woocommerce->cart->get_cart_subtotal()); ?></span></span>
                                    <a href="<?php echo esc_url($woocommerce->cart->get_checkout_url()); ?>" class="btn btn-primary btn-checkout"><?php _e( 'Process To Checkout', 'zo-mercury'); ?></a>
                                    <a href="<?php echo esc_url($woocommerce->cart->get_cart_url()); ?>" class="btn btn-cart"><?php _e( 'View Shopping Cart', 'zo-mercury'); ?></a>
                                </div>
                            <?php endif; ?>
                        </div>
                ...
                <?php if($show_search):?>
                    <div class="widget_searchform_content">
                        <form method="get" action="<?php echo esc_url( home_url( '/'  ) );?>">
                            ...
                            <input type="submit" value="<?php echo esc_attr__( 'Search', 'zo-mercury' )?>" />
                            <?php if($woocommerce):?>
    ...
    function form( $instance ) {
		...
		<p>
            <label for="<?php echo esc_url($this->get_field_id('show_cart')); ?>"><?php _e( 'Show Cart:', 'zo-mercury' ); ?></label>
			<select id="<?php echo esc_attr($this->get_field_id('show_cart')); ?>" class="widefat" name="<?php echo esc_attr($this->get_field_name('show_cart')); ?>">
				<option value="0" <?php selected($show_cart,0); ?>><?php echo __('No','zo-mercury'); ?></option>
				<option value="1" <?php selected($show_cart,1); ?>><?php echo __('Yes','zo-mercury'); ?></option>
			</select>
        </p>
		<p>
            <label for="<?php echo esc_attr($this->get_field_id('show_search')); ?>"><?php _e( 'Show Search:', 'zo-mercury' ); ?></label>
			<select id="<?php echo esc_attr($this->get_field_id('show_search')); ?>" class="widefat" name="<?php echo esc_attr($this->get_field_name('show_search')); ?>">
				<option value="0" <?php selected($show_search,0); ?>><?php echo __('No','zo-mercury'); ?></option>
				<option value="1" <?php selected($show_search,1); ?>><?php echo __('Yes','zo-mercury'); ?></option>
			</select>
        </p>
    <?php
