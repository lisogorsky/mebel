...
<div class="zo-grid-wraper <?php echo esc_attr($atts['template']);?>" id="<?php echo esc_attr($atts['html_id']);?>">
        ...
        <div class="zo-filter-wrapper clearfix">
            <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                <div class="zo-grid-filter clearfix">
                    <span><?php echo esc_html__('Sort by','zo-mercury'); ?></span>
                    <select class="zo-filter-category list-unstyled list-inline">
                    <option data-group="all"><?php esc_html_e("All", 'zo-mercury');?></option>
                    <?php
                        $posts = $atts['posts'];
                        $query = $posts->query;
                        $taxs  = array();
            ...
            <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                    <div class="zo-grid-filter clearfix">
                        ...
                        <select name="order" class="zo-filter-order list-unstyled list-inline">
                            <option value=""><?php echo esc_html__('Default','zo-mercury');  ?></option>
                            <option value="order-newest"><?php echo esc_html__('Newest','zo-mercury');?></option>
                            <option value="order-title"><?php echo esc_html__('Title','zo-mercury');?></option>
                        </select>
                    </div>
            </div>
    ...
    <div class="row zo-grid <?php echo esc_attr($atts['grid_class']);?>">
        ...
        while($posts->have_posts()){
            ...
            $groups = array();
            $groups[] = '"all"';
			$categories = zoGetCategoriesByPostID(get_the_ID(),$taxonomy);
